...
const getOrderList = async () => {
	...
	orderlistData.value = JSON.parse(res.data.data).data.orders.filter((item:any)=>{
...
<template>
	...
	<el-drawer v-model="drawer" :direction="direction" :size="isPhone? '100%': '700px'">
		<template #header>
		...
		<div class="allpromotion">
			...
			<div class="orderBox">
				...
				<div class="orderInfo">
					...
					<p v-if="orderDetailData?.orderInfo?.pay_id">支付单号：{{ orderDetailData?.orderInfo?.pay_id }}</p>
				</div>
			</div>
			<div class="orderCommodity">
				...
				>
					...
					<el-table-column prop="specification_name" label="规格" min-width="70">
					</el-table-column>
			...
			<el-table
				...
				:show-header="false"
			>
				<el-table-column prop="goods_name" label="商品名称" :width="isPhone? 100 : 230">
					<template #default="scope">
