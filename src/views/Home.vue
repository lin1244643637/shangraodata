...
const getorderData = async () => {
  ...
  console.log(
    JSON.parse(response.data.data).data.orders.filter((item: any) => {
      return (
        item.address.indexOf("上饶") > -1 &&
        item.order_status_text == "已取消" &&
        item.pay_status == 1
      );
    }),
  );
  orderlistData.value = JSON.parse(response.data.data).data.orders.filter(
    (item: any) => {
  ...
  <div class="home">
    <el-card class="card">
      ...
      <div style="text-align: left">
        <p v-for="item in salesTotal" :key="item.date">
    ...
    <el-card class="card">
      ...
      <el-table
        :data="[...homeData].sort((a, b) => b.dayNewScans - a.dayNewScans)"
        style="width: 100%"
        border
      >
    ...
    <el-card class="card">
      ...
      <el-table
        :data="[...homeData].sort((a, b) => b.weekNewScans - a.weekNewScans)"
        style="width: 100%"
        border
      >
        <el-table-column type="index" width="50" />
    ...
    <el-card class="card">
      ...
      <el-table
        :data="[...homeData].sort((a, b) => b.monthNewScans - a.monthNewScans)"
        style="width: 100%"
        border
      >
        <el-table-column type="index" width="50" />
...
function $currency(goods_original_price: any) { throw new Error('Function not
implemented.'); }
